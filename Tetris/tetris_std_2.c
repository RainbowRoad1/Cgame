...
#define W 7
#define H 15
const int ALL = (4 << W) - 1, AIR = 2 << W | 1, CL = W + 2;
char o[(W + 2) * H], *c = o + (W + 2) * H, S[] = "IOJLTZN";
...
void fill(char c1) {
    for (c = (y - 2) * CL + x + 3 + o, i = 0; i < 4; ++i)
        for (c += CL - 4, j = 0; j < 4; ++j, ++c)
            if (n[t][i] >> j & 1) *c = c1;
}
void move(int *v, int w) {
    for (*v += w, i = 0; i < 4 && !(m[i + y] & (n[t][i] << x));) ++i;
    if (i == 4 || (*v -= w, v != &y) || (x | y) == 1 && !(y = 0)) return;
    for (fill(S[t >> 2]), i = 4; i--;) m[y + i] |= n[t][i] << x;
...
int main() {
    ...
    for (o[CL * H - 1] = '\0', srand((unsigned)time(0)); y; _sleep(100)) {
        ...
        tot = 0, fill(S[t >> 2]), system("cls"), printf(o);
    }
    printf("\nGame over!\n");
}
