#include <stdlib.h>
#include <conio.h>
int W = 15, S, i, c, z, e, *m, *p, l[4] = {1, -1};
...
void init(){
	for (i = S; i-- || (m[i = rand() % S] - 2 ? i = S : dig(m + i), 0);)
		m[i] = 2, (i + 1) % W < 2 | i % (S - W) < W && ++m[i];
	for (i - S && (i = 0); (i += W) < S && m[i] & 3 | m[S - 1 - i] & 3;);
	i >= S ? init() : (m[z = i] = m[e = S - 1 - i] = 0);
}
int main(){
	srand(m = calloc(S = W * W, 4)), l[2] = W, l[3] = -W, c = 81;
